...
impl FqCrypto {
	...
	//println!("server_device_id.to_ne_bytes():{:?}", server_device_id.to_ne_bytes());
        let mut iv = [0u8; 16];  // Initialize an array with 16 bytes
        rand::thread_rng().fill(&mut iv);  // Fill the array with random bytes
	println!("iv:{:?}", iv);

	println!("aaaa:{:?}",combined_bytes.as_slice());
	//let numbers: Vec<String> = combined_bytes.iter().map(|&byte| byte.to_string()).collect(); // 将每个字节转换为字符串数字
...
impl FqIBatchFullResponse {
    pub async fn get_decrypt_contents(&self, client: &Client, var: &FqVariable) -> anyhow::Result<Vec<(String, String, String)>> {
	...
	//println!("{:#?}", &self.data);
	    ...
	    //println!("\nid:\t{}\ntitle:\t{}\ncontent:{}", item_id, content.title, s);
	    //println!("{:?}", content);
            res.push((item_id.clone(), content.title.clone(), s));
        }
        Ok(res)
    }
}